...
function SendQuotation() {
    ...
    const formInitialDetails = {
      ...
      const [buttonText, setButtonText] = useState('Send Quotation');
      ...
      const handleSubmit = async (e) => {
        ...
        setButtonText('Send Quotation');
        let result = await response.json();
        setFormDetails(formInitialDetails);
        if (result.code === 200) {
          setStatus({ succes: true, message: 'Sent Successfully !' });
        } else {
          setStatus({
            succes: false,
              ...
              <Col size={12} md={6}>
                <TrackVisibility>
                  {({ isVisible }) => (
                    ...
                    >
                      <h2 className='text-center'>Send Quotation</h2>
                      <form onSubmit={handleSubmit}>
                        ...
                        <input
                          ...
                          value={formDetails.amountPerItem}
                          placeholder='Amount Per Item'
                          onChange={(e) => onFormUpdate('amountPerItem', e.target.value)}
                          required
                        />
                        ...
                        <input
                          ...
                          onChange={(e) => onFormUpdate('totalAmount', e.target.value)}
                          required
                        />
